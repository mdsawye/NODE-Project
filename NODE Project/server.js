...
router.get('/user', function(req, res) {
    res.json({ message: 'hooray! welcome new user!' });   
});
...
router.route('/bears')
    ...
    .post(function(req, res) {
        ...
        var bear = new bearmodel();      // create a new instance of the Bear model
...
router.route('/bears/:bear_id')
  ...
  // get the bear with that id (accessed at GET http://localhost:8080/api/bears/:bear_id)
    .get(function(req, res) {
        bearmodel.findById(req.params.bear_id, function(err, bear) {
            if (err)
                res.send(err);
            res.json(bear);
        });
